...
class Weather:
    ...
    def predict(self, temp, humidity, forecast):
        ...
        for i in range (0, predicted_weather_condition.shape[0]):
            predicted_weather_condition[i] = np.format_float_positional(np.float32(predicted_weather_condition[i]), unique=False, precision=2)
