...
class Graphs:
    ...
    def __init__(self, widget):
        ...
        pl_view.resize(361, 241)
        pl_layout = pg.GraphicsLayout()
        pl_view.setCentralItem(pl_layout)
        pl_graph = pl_layout.addPlot(title="Görev Yükü")
        pl_graph.addLegend()
        pl_graph.setXRange(0, 20, padding=0)
        pl_graph.vb.setLimits(xMin=0, xMax=20)
        ...
        self.pl_lat_data = np.linspace(0, 0, 20)
        self.pl_lon_data = np.linspace(0, 0, 20)
        self.pl_alt_data = np.linspace(0, 0, 20)

        # Carrier latitude, longitude and altitude graph
        car_view = pg.GraphicsView(self.widget.graph_car)
        car_view.resize(361, 241)
        car_layout = pg.GraphicsLayout()
        car_view.setCentralItem(car_layout)
        car_graph = car_layout.addPlot(title="Taşıyıcı")
        car_graph.addLegend()
        car_graph.setXRange(0, 20, padding=0)
        car_graph.vb.setLimits(xMin=0, xMax=20)
        ...
        self.car_lat_data = np.linspace(0, 0, 20)
        self.car_lon_data = np.linspace(0, 0, 20)
        self.car_alt_data = np.linspace(0, 0, 20)
